...
flowchart TB
    subgraph Desktop["open_emby — Electron 桌面端 (open_emby/)"]
        ...
        CF["ComfyUI Connector<br/>HTTP + WebSocket 客户端"]
        SC["Sidecar Supervisor<br/>Node 进程守护 / 健康检查"]
    end

    subgraph Local["本地推理与制版后端"]
        CU["ComfyUI (D:/comfyui)<br/>图像生成 · 风格化 · 局部重绘"]
...
| 服务 | 职责 |
|---|---|
| `comfyui.ts` | 连接 D:/comfyui 的 API（默认 127.0.0.1:8188），提交工作流 JSON、WebSocket 进度、结果取回；自动探测/拉起 ComfyUI |
| `modelManager.ts` | 从 HuggingFace 下载 checkpoint/LoRA 到 `models/`，生成 ComfyUI 软链/extra_model_paths；微调任务（kohya sd-scripts / ComfyUI LoRA 训练节点）队列管理 |
...
- **职责切分**：UI 与生态编排 = Electron/Node；性能密集计算（编解码、加解密、图像处理、文件索引）= Rust（napi-rs 原生模块 `crates/emby-core`）。
...
- **制版流水线 digitizer**：图像分割 → 颜色量化（限色到线色数）→ 针法分配（平针/缎面/榻榻米/轮廓）→ 密度与方向 → 针迹路径生成（热路径逐步下沉 Rust）。
- **导出 exporters**：当前 DST（Rust 编码器，自研替代 pyembroidery）；PES/JEF/EXP 编解码器按同模式在 emby-core 扩展。
- **Agent**：工具调用循环（本地小模型 GGUF via llama.cpp，或远程 API），工具 = {图像分析, ComfyUI 工作流, 制版参数, 导出}，产出可解释的制版方案。
- **例外**：模型微调执行器（kohya sd-scripts 等）允许使用 Python，但隔离在独立训练工具链中，不属于产品运行时。

### 4.3 插件系统
- manifest（plugin.json）+ 入口模块，类型：`workflow`（ComfyUI 工作流包）、`exporter`（新机器格式）、`tool`（编辑器工具）、`agent-skill`（Agent 能力）、`theme`。
...
- 数据集在 `E:/Project-刺绣机/datasets`，训练脚本读取该目录，产物 LoRA 写回 `models/loras`，Model Manager 注册后 ComfyUI 立即可用。
...
## 6. 环境约定
- ComfyUI: `D:\comfyui`，API 默认 `http://127.0.0.1:8188`
