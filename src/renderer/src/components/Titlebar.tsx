  ...
  return (
    <div className="titlebar">
      ...
      <div className="titlebar-controls">
        <button className="win-btn" title="最小化" onClick={() => window.openEmby.window.minimize()}>
          <svg width="12" height="12" viewBox="0 0 12 12"><line x1="1" y1="6" x2="11" y2="6" stroke="currentColor" strokeWidth="1.4"/></svg>
        </button>
        <button className="win-btn" title={maximized ? '还原' : '最大化'} onClick={() => window.openEmby.window.toggleMaximize()}>
          {maximized ? (
        ...
        <button className="win-btn close" title="关闭" onClick={() => window.openEmby.window.close()}>
